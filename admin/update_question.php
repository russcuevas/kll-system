...
if (isset($_GET['id'])) {
    ...
    $sql = "
        SELECT q.id AS question_id, q.question_text, GROUP_CONCAT(c.id ORDER BY c.course_name ASC) AS course_ids
    ...
    $stmt_get_courses = $conn->query($get_courses);
    $courses = $stmt_get_courses->fetchAll(PDO::FETCH_ASSOC);

    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $updated_question_text = $_POST['question_text'];
        $selected_course_ids = $_POST['course_id'];

        $update_sql = "UPDATE tbl_questions SET question_text = :question_text, updated_at = NOW() WHERE id = :question_id";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bindParam(':question_text', $updated_question_text);
        $update_stmt->bindParam(':question_id', $question_id);
        $update_stmt->execute();

        $delete_sql = "DELETE FROM tbl_question_courses WHERE question_id = :question_id";
        $delete_stmt = $conn->prepare($delete_sql);
        $delete_stmt->bindParam(':question_id', $question_id);
        $delete_stmt->execute();

        foreach ($selected_course_ids as $course_id) {
            $insert_sql = "INSERT INTO tbl_question_courses (question_id, course_id, created_at, updated_at) VALUES (:question_id, :course_id, NOW(), NOW())";
        ...
        header("Location: update_question.php?id=$question_id");
        exit();
    }
} else {
    header('Location: questionnaire.php');
...
<body class="theme-red">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        ...
                        <div class="body">
                            ...
                            <form id="update_question_validation" action="update_question.php?id=<?= $question['question_id'] ?>" method="POST" enctype="multipart/form-data">
                                        ...
                                        <select name="course_id[]" class="form-control show-tick" multiple data-live-search="true" required>
                                            <?php foreach ($courses as $course): ?>
                                                <option value="<?= $course['id'] ?>"
                                                    <?= in_array($course['id'], explode(',', $question['course_ids'])) ? 'selected' : ''; ?>>
                                                    <?= $course['course_name'] ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                ...
                                <button class="btn btn-primary waves-effect" type="submit">Update Question</button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(function() {
            ...
            $('#update_question_validation').validate({
                rules: {
                    'date': {
                        customdate: true
                    },
